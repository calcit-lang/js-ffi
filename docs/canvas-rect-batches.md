...
旧 `draw-rects!`、`draw-transformed-clipped-rects!` 和 `canvas-rect-batches.mjs` 已迁出 js-ffi。Quamolit 的真实 10k 实例消费者现在通过 Calcit `quamolit.instance-ffi/draw-canvas!` 调用 Quamolit 仓库的 `src/host/canvas-rect-batches.mjs`；该宿主循环保留一次批次提交、预检、状态恢复和逐矩形 Canvas 调用。旧 `CanvasRectMetrics` 仍暂留为 Calcit 兼容类型，待 Quamolit 自有类型迁移后清理。
