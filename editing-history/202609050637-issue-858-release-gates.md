...
- 禁止 checkout 持久化 GitHub token；后续门禁均为只读，不需要认证后的 Git 操作。
...
- Disable checkout credential persistence because all later gates are read-only and require no authenticated Git operations.
