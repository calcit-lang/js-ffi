# RFC 0001: 以 Calcit 类型系统为中心的 JavaScript FFI 边界
...
2. 函数的 `:features` 必须成为完整函数类型信息的一部分，在函数值、回调和类型匹配中
   不得丢失。
3. external-object trait 需要有限的参数化能力，才能表达 `Promise<T>`、
   `Event<Target>`、`Iterator<T>` 等宿主类型关系。
4. 宿主字段写入需要显式 writable 契约；默认 external field 只读。
5. FFI wrapper 应尽快把宿主值规范化为 Calcit `Struct`、`Enum`、`Option`、`Result`
   和 `Unit`，避免把 `Dynamic`、裸 Map 或 `JsObject` 扩散到业务代码。

本 RFC 同时规定 `js-ffi` 仓库的建模原则，以及需要在 Calcit 编译器中渐进实现的检查。
...
这会导致一个带 `:js-ffi` 的函数值在通过回调、高阶函数或缓存后的类型信息中失去
capability evidence。对于第一阶段的 named function body 检查影响有限，但无法支撑后续
严格隔离和 target compatibility。

### external trait 无法表达类型参数关系
...
若公共 API 不暴露这些宿主对象，可以继续用 typed wrapper 规避；若要形成可复用的
FFI 类型库，则需要参数化 external trait，或者等价的 associated type 机制。
...
- `E_JS_FFI_CALLBACK_FEATURES`：回调 capability 不满足预期 contract。
...
所有从 schema、函数值、import 或 callback hint 构造 `Fn` 类型的路径都必须保留
features。`Eq`、`Ord`、`Hash`、query 和 cache key 应包含 feature set。

回调兼容规则建议为：

```text
actual callback features ⊆ expected callback allowed features
```

也就是说，允许 `:js-ffi` 的 callback slot 可以接收纯 callback；声明为纯的 callback
slot 不能接收内部执行 raw JS 的 callback。

普通 direct call 不要求 caller 拥有 callee 的 features；raw operation 仍然只发生在
callee 已标记的实现体内。若未来需要表达传递副作用，应新增独立的 `:effects`，不要把
...
建议支持与 generic Struct/Enum 一致的声明形式：

```cirru.no-check
deftrait JsPromise ('T)
  .then! $ :: 'Fn
    {}
      :generics $ [] 'U
      :args $ []
        :: 'js-ffi.async/JsPromise 'T
        :: 'Fn $ {}
          :args $ [] 'T
          :return 'U
      :return $ :: 'js-ffi.async/JsPromise 'U
```

同样可表达 `JsIterator<T>`、`JsEvent<Target>` 和 `JsMap<K, V>`。实现要求：

- `CalcitTrait` 保存 generic parameters；
- `CalcitTypeAnnotation::Trait` 支持 applied type args，或增加等价 applied-trait 形态；
- member type lookup 替换 trait parameters；
- `:where`、TraitSet、type matching、query 和 codegen hint 保留 applied args；
- external field/method inference 使用替换后的具体类型。

这是一项较大的扩展，应在实际需要公开 `Promise<T>` 等 host handle 时实现。在此之前，
推荐让 async wrapper 直接返回 `Result<T, JsError>`，不要公开无类型 Promise。
...
- `JsArray<T>`：优先在边界转换成 `List<T>`；需要保留宿主数组时再用参数化 trait；
- `JsPromise<T>`：等待参数化 trait，或在 adapter 中 await 后返回 `Result<T, E>`；
...
- 为 Storage、Document、Location、Process 等稳定能力声明 external trait；
...
### Phase 2：函数 feature 完整性

- features 进入 Fn equality/hash/cache/query；
- runtime function 与 anonymous callback 保留 features；
- 检查可逃逸 closure 与 callback feature compatibility；
- effects graph 单独记录传递 effects，不改变 `:features` 的 body capability 语义。

### Phase 3：按真实需求扩展 host type relation

- generic external trait；
- writable field metadata；
- target availability check；
- 只有出现无法通过 wrapper/Enum/Struct 表达的真实 API 后，再评估 associated type 或
  其他类型扩展。
...
7. 带 `:js-ffi` 的匿名 callback 不会在函数类型传播中丢失 feature。
...
- generic external trait；
- target-specific applied host handles；
