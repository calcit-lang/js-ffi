...
import {
  ...
  positions_from_list as positionsFromList, read_translation_$x_ as readTranslation,
...
export async function testCalcitWebGpuBatches(a) {
  ...
  const batch = {
    ...
    draw(options) {
      submitted = options;
      return { drawCalls: 1, instances: 2, positionBytesUploaded: 0, uniformBytesUploaded: 64 };
    },
  ...
  const frame = struct(RectFrame, { width: 8, height: 6, fill, alpha: 0.75, translation: some(motion) });
  ...
  a.equal(JSON.stringify(submitted), JSON.stringify({
    ...
    translation: { from: { x: 1.25, y: -2.5 }, to: { x: 17.75, y: 8.25 }, time: 0.5, start: 0, duration: 1, easing: 'smoothstep' },
  }));
  a.equal(toJsData(await readTranslation(batch)).x, 3.5);
  a.equal(toJsData(await readTranslation(batch)).y, -2.25);
  drawRects(batch, struct(RectFrame, { width: 8, height: 6, fill, alpha: 1, translation: none() }));
