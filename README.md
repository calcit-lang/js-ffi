...
## 0.2.0-alpha.4 模块内 JS 文件候选

此候选版要求 Calcit `0.22.0-alpha.3`。`js-ffi.browser/document-available?` 的实现位于模块根目录的 `js-ffi-assets/document-available.js`，仍带有 `Fn [] -> Bool` schema 与 `:js-ffi` 标记。Calcit 编译时把文件中的单个函数表达式嵌入生成模块；下游继续通过普通 Calcit `:require` 调用，不需要单独引用 JS 文件或安装片段专用 npm 包。
